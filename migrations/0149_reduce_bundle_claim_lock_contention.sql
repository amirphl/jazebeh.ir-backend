-- Replace the former per-audience advisory claim lock with one lock per
-- Bundle. This keeps cross-table claim validation safe for direct writers,
-- while avoiding an advisory lock per selected audience and lock-order
-- deadlocks for overlapping bulk reservations.
...
BEGIN
    IF TG_TABLE_NAME IN ('campaign_targeting_test_sample_reservations', 'campaign_targeting_execution_reservations') THEN
        IF NEW.state <> 'active' THEN
            RETURN NEW;
        END IF;
        PERFORM pg_advisory_xact_lock(845172, NEW.bundle_id);
        IF EXISTS (
