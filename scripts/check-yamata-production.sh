...
[[ "$(env_value yamata-app-beta CAMPAIGN_REFUND_RECONCILIATION_SCHEDULER_ENABLED)" == true ]] ||
	die "Refund reconciliation must be enabled in yamata-app-beta"
[[ "$(env_value yamata-campaign-scheduler-beta CAMPAIGN_REFUND_RECONCILIATION_SCHEDULER_ENABLED)" == false ]] ||
	die "Refund reconciliation must be disabled in the campaign scheduler"
