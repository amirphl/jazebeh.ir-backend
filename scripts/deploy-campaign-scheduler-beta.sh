...
"${DOCKER[@]}" inspect -f '{{range .Config.Env}}{{println .}}{{end}}' "$SOURCE_CONTAINER" |
	awk -F= '
		BEGIN {
			...
			override["BUNDLE_ACTION_FILE_SCHEDULER_ENABLED"] = 1
			override["BUNDLE_ACTION_FILE_SCHEDULER_INTERVAL"] = 1
			override["BUNDLE_ACTION_FILE_SCHEDULER_MAX_PARALLEL_RUNS"] = 1
...
# Bundle action files live on the API container's upload volume.  Keep their
# worker co-located with that API process; this isolated campaign worker must
# never claim those jobs.
BUNDLE_ACTION_FILE_SCHEDULER_ENABLED=false
BUNDLE_ACTION_FILE_SCHEDULER_INTERVAL=1m
BUNDLE_ACTION_FILE_SCHEDULER_MAX_PARALLEL_RUNS=2
