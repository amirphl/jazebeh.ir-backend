...
func TestSmartTargetingPopulationUsesOptionalParameterizedColorFilter(t *testing.T) {
	...
	for _, required := range []string{"bundle_audience_selection_members", "used.bundle_id", "used.audience_id", "campaign_targeting_test_sample_reservations", "campaign_targeting_execution_reservations", "campaign_targeting_execution_calculations", "campaign_targeting_execution_calculation_members", "execution_calculation.status = 'committed'", "reserved.state = 'active'", "bundle_audience_exclusions", "bundle_exclusion.bundle_id", "bundle_exclusion.audience_id", "percentile_disc", "ap.color = any(?::text[])"} {
		if !strings.Contains(query, required) {
			t.Fatalf("capacity population is missing %q:\n%s", required, smartTargetingPopulationCTE)
		}
	}
	for _, population := range []string{smartTargetingBasePopulationCTE, smartTargetingSelectionBasePopulationCTE} {
		if !strings.Contains(population, "AND NOT EXISTS (\n          SELECT 1\n          FROM bundle_audience_exclusions") || strings.Contains(population, "?::boolean OR NOT EXISTS") {
			t.Fatalf("Bundle exclusions must be unconditional for every selection phase:\n%s", population)
		}
	}
...
func TestSmartTargetingPopulationArgsKeepBundleExclusionsMandatory(t *testing.T) {
	args := smartTargetingPopulationArgs(SmartTargetingAudienceQuery{TagIDs: []int64{7}, BundleID: 3, Phase: SmartTargetingSelectionPhaseExecution})
	...
	args = smartTargetingPopulationArgs(SmartTargetingAudienceQuery{
		TagIDs: []int64{7}, BundleID: 3, AllowedColors: []string{"white", "pink"}, Phase: SmartTargetingSelectionPhaseExecution,
	})
	if disabled, ok := args[1].(bool); !ok || disabled {
		t.Fatalf("SMS allowed colors produced disabled=%#v, want false", args[1])
	}
	if len(args) != 14 || args[3] != uint(3) || args[4] != uint(3) || args[5] != uint(0) || args[6] != uint(0) || args[7] != uint(3) || args[8] != uint(0) || args[9] != uint(0) || args[10] != uint(3) || args[11] != uint(0) || args[12] != uint(0) || args[13] != uint(3) {
		t.Fatalf("population arguments = %#v, want colors and Bundle eligibility inputs", args)
	}
}

func TestSmartTargetingAudienceQueryRequiresExplicitSelectionPhase(t *testing.T) {
	query := SmartTargetingAudienceQuery{BundleID: 3, TagIDs: []int64{7}, ScoreClasses: []string{"A"}}
	if query.valid() {
		t.Fatal("selection query without a phase must fail closed")
	}
	for _, phase := range []SmartTargetingSelectionPhase{SmartTargetingSelectionPhaseTest, SmartTargetingSelectionPhaseExecution} {
		query.Phase = phase
		if !query.valid() {
			t.Fatalf("selection query rejected explicit %q phase", phase)
		}
	}
}
...
func TestSmartTargetingScoreBoundsQueryCalculatesEligibleUnionOnce(t *testing.T) {
	query := SmartTargetingAudienceQuery{
		BundleID: 3, Phase: SmartTargetingSelectionPhaseTest, TagIDs: []int64{9, 2}, ScoreClasses: []string{"A", "C"}, AllowedColors: []string{"white", "pink"},
	}
	...
	for _, required := range []string{
		...
		"campaign_targeting_test_sample_reservations", "campaign_targeting_execution_reservations", "reserved.state = 'active'",
	...
	withoutColors, args := smartTargetingScoreBoundsQuery(SmartTargetingAudienceQuery{
		BundleID: 3, Phase: SmartTargetingSelectionPhaseTest, TagIDs: []int64{9}, ScoreClasses: []string{"A"},
	})
	if strings.Contains(strings.ToLower(withoutColors), "ap.color") || len(args) != 12 {
	...
	if !strings.Contains(strings.ToLower(withoutColors), "bundle_audience_exclusions") {
		t.Fatalf("score-bound query must apply Bundle exclusions:\n%s", withoutColors)
	}
}
...
func TestSmartTargetingPerTagSelectionUsesNoOrdering(t *testing.T) {
	p33, p66 := 24.0, 29.6
	query := SmartTargetingAudienceQuery{
		BundleID: 3, Phase: SmartTargetingSelectionPhaseTest, TagIDs: []int64{9, 2}, ScoreClasses: []string{"A", "C"},
	}
	...
	if len(args) != 16 || args[0] != int64(9) || args[1] != uint(3) || args[2] != uint(3) {
...
func TestSmartTargetingPerTagSelectionPreservesAllClassColorEligibility(t *testing.T) {
	query := SmartTargetingAudienceQuery{
		BundleID:      3,
		Phase:         SmartTargetingSelectionPhaseTest,
		TagIDs:        []int64{9},
	...
	for _, required := range []string{
		...
		"campaign_targeting_execution_reservations",
	...
	if len(args) != 15 || args[0] != int64(9) || args[2] != uint(3) || args[3] != uint(3) || args[6] != uint(3) || args[9] != uint(3) || args[12] != uint(3) {
...
func TestSmartTargetingAudienceQueriesCanKeepOwnActiveTestReservation(t *testing.T) {
	query := SmartTargetingAudienceQuery{
		BundleID: 3, Phase: SmartTargetingSelectionPhaseTest, ExcludeActiveTestReservationCampaignID: 17,
