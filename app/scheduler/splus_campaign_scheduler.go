...
func (s *SplusCampaignScheduler) processSplusCampaign(ctx context.Context, jazzAccessToken string, c dto.BotGetCampaignResponse) (err error) {
	...
	if usesExcelAudienceTargeting(c) {
		...
		s.logger.Printf("Splus scheduler: campaign id=%d fetched %d phones (bundle_audience_selection_id=%d)", c.ID, len(phones), *bundleAudienceSelectionID)
	}
...
func (s *SplusCampaignScheduler) selectSplusTagAudiences(
	...
	// Splus intentionally does not segment audiences by color.
	...
	s.logger.Printf("selectSplusTagAudiences candidates: campaign_id=%d count=%d limit=%d excluded=%d", campaignID, len(candidates), limit, len(excludeIDs))
...
func (s *SplusCampaignScheduler) fetchSplusAudiencePhonesByBundle(
	...
	if usesSmartAudienceTargeting(c) {
		...
		phones, ids, uids, selectionID, err = selectAndReserveStandardBundleCandidates(
			...
			func(selectionCtx context.Context, exclude map[int64]struct{}) ([]string, []int64, []string, error) {
				return s.selectSplusTagAudiences(selectionCtx, c.ID, tagIDs, numAudiences, exclude, &bundleID, scoreConstraint)
			},
	...
	if len(phones) == 0 {
		return &AudiencePhonesResult{
			...
			IDs:    ids,
			UIDs:   uids,
			Codes:  []string{}, BundleAudienceSelectionID: utils.ToPtr(selectionID),
		}, nil
	}
